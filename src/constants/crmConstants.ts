export const STATUS_OPTIONS = [
  { name: 'Chờ phản hồi', color: 'bg-amber-50 text-amber-700 border-amber-200' },
  { name: 'Đã liên hệ', color: 'bg-sky-50 text-sky-700 border-sky-200' },
  { name: 'Hẹn phỏng vấn', color: 'bg-blue-50 text-blue-700 border-blue-200' },
  { name: 'Trúng tuyển', color: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
  { name: 'Từ chối / Block', color: 'bg-rose-50 text-rose-700 border-rose-200' },
];

export const CATEGORY_OPTIONS = [
  { value: 'F&B', label: '🍔 Food & Beverage (F&B)', color: 'bg-amber-50 text-amber-700 border-amber-200' },
  { value: 'IT', label: '💻 Công nghệ thông tin (IT)', color: 'bg-sky-50 text-sky-700 border-sky-200' },
  { value: 'Logistics', label: '📦 Vận chuyển / Logistics', color: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
  { value: 'Sales & Marketing', label: '📈 Sales & Marketing', color: 'bg-fuchsia-50 text-fuchsia-700 border-fuchsia-200' },
  { value: 'Others', label: '💼 Ngành nghề khác', color: 'bg-slate-100 text-slate-600 border-slate-200' }
];
// // === DỮ LIỆU MẪU BAN ĐẦU ===
...
// export const GOOGLE_APPS_SCRIPT_CODE = `// === MÃ APPS SCRIPT ĐÃ ĐƯỢC SỬA LỖI TYPO VÀ ĐỒNG BỘ HÓA CORS ===
// function doGet(e) {
...
//     var obj = {}; // Đã xóa chữ 'c' bị thừa gây lỗi cú pháp ở đây
...
//     var job = jobsList[i];
//     sheet.appendRow([
//       job.id||"", job.title||"", job.shopName||"", job.phone||"", job.address||"", 
//       job.link||"", job.salary||"", job.requirements||"", job.benefits||"", 
//       job.status||"Chờ phản hồi", job.notes||"", job.dateAdded||""
//     ]);
//   }
